...
def GridSearch(data_set, KernelLen, KernelNum, RandomSeed, Modeltype, path,GPUID="0",
    ...
    elif Modeltype == "Markonv":
        trainMarkonv(path, data_set, KernelNum, KernelLen,
                     RandomSeed, batch_size, epoch_scheme,GPUID=GPUID,outputName="Markonv")
    elif Modeltype == "MarkonvV":
        trainMarkonv(path, data_set, KernelNum, KernelLen,
                     RandomSeed, batch_size, epoch_scheme, GPUID=GPUID, outputName="MarkonvV")
    elif Modeltype == "HOCNNLB":
        trainHOCNNLB(path, data_set, KernelNum, KernelLen,
                     RandomSeed, batch_size, epoch_scheme, GPUID=GPUID, outputName="HOCNNLB")
    else:
...
if __name__ == '__main__':
    ...
    Outpath = DataPath.replace("external", "result").replace("Hdf5","")
    mkdir(Outpath)
