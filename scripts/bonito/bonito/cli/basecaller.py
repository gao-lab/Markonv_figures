...
def main(args):
    ...
    if args.tr:
        from bonito.litetr.basecall import basecall
    else:
        if args.verbose:
    ...
    if not args.tr:
        config_batchsize = model.config["basecaller"]["batchsize"]
        config_chunksize = model.config["basecaller"]["chunksize"]
        config_overlap = model.config["basecaller"]["overlap"]
    else:
        config_batchsize = args.batchsize
        config_chunksize = args.chunksize
        config_overlap = args.overlap
    ...
    results = basecall(
        ...
        chunksize=config_chunksize,
        overlap=config_overlap,
        discrete=args.discrete, half=not args.no_half,
    )
