...
def linear_warmup_linear_cooldown(end_ratio=0.0, warmup_steps=500, **kwargs):
    """
    Linear warmup, linear Cooldown Scheduler
    """
    return lambda optimizer, train_loader, epochs, last_epoch: func_scheduler(
        optimizer=optimizer,
...
def linear_warmup_inverse_sqrt_decay(end_ratio=0.0, warmup_steps=500, **kwargs):
    """
    Linear warmup, inverse sqrt decay scheduler
    """
    return lambda optimizer, train_loader, epochs, last_epoch: func_scheduler(
        optimizer=optimizer,
        func=inverse_sqrt_decay_schedule(1.0),
        total_steps=epochs * len(train_loader),
        warmup_steps=warmup_steps,
        start_step=last_epoch * len(train_loader),
    )
