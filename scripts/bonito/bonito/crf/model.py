...
from bonito.nn import Module, Convolution, LinearCRFEncoder, ResNet, Serial, Permute, layers, from_dict, MarkonvLayer, Embedding, AddPositionalEmbedding
...
def get_stride(m):
    ...
    if isinstance(m, MarkonvLayer):
        return get_stride(m.markonv)
...
def conv(c_in, c_out, ks, stride=1, bias=False, activation=None, bn=False, **kwargs):
    return Convolution(c_in, c_out, ks, stride=stride, padding=ks//2, bias=bias, activation=activation, bn=bn)


def mar(c_in, c_out, ks, stride=1, bias=False, activation=None, bins=20,v=0, channel_last=False,bn=False, residue=False, **kwargs):
    if ks %2 == 0:
        pad = (ks//2-1, ks//2-1)
    else:
        pad = (ks//2-1, ks//2)
    return MarkonvLayer(c_in, c_out, ks, stride=stride, padding=pad, bias=bias, activation=activation, bins=bins,v=v, channel_last=channel_last, bn=bn, residue=residue, **kwargs)


def stackrnn(features, num_layers, rnn_type, transformer_activation, **kwargs):
    rnn = layers[rnn_type]
    if rnn_type in ["transformer"]:
        # NCT -> NTC -> TNC
        return [Permute([0, 2, 1])]+list(
            (rnn(features, features, reverse=False, activation=transformer_activation, **kwargs) for i in range(num_layers))
            )+[Permute([1, 0, 2])]
    elif rnn_type in ["pretrainedtransformer"]:
        # NCT -> NTC -> TNC
        return [Permute([0, 2, 1])]+[rnn(features, features, reverse=False, activation=transformer_activation, **kwargs)
        ]+[Permute([1, 0, 2])]
    elif rnn_type in ["litetransformer"]:
        # NCT -> NTC -> TNC
        return [Permute([0, 2, 1])]+[rnn(features, features, reverse=False, activation=transformer_activation, num_layers=num_layers, **kwargs)
        ]+[Permute([1, 0, 2])]
    elif rnn_type == "bilstm":
        # NCT -> TNC
        return [Permute([2, 0, 1])]+list(
            (rnn(features, features//2, reverse=False, activation=transformer_activation, **kwargs) for i in range(num_layers)))
    elif rnn_type == "lstm":
        # NCT -> TNC
        return [Permute([2, 0, 1])]+list(
            (rnn(features, features, reverse=(num_layers - i) % 2, activation=transformer_activation, **kwargs) for i in range(num_layers)))
    else:
        raise NotImplementedError


def rnn_encoder(n_base, state_len, insize=1, stride=5, winlen=19, activation='swish', rnn_type='lstm', features=768, scale=5.0, blank_score=None, expand_blanks=True, num_layers=5, markonv=0,variable=0, hidden_channel=16, transformer_activation='relu', embedding_size=[1,5,20], last_batchnorm=False, residue=False, bn=True, kernel_size="default"):
    rnn = layers[rnn_type]
    if markonv == 0:
        return Serial([
                # NCT: bs, 1, len
        ...
        # last_activation of embedding is true, last_batchnorm is True in this version.
        # bn is true, residue is true.
        # markonv-residue-markonv
        return Serial([
                Embedding(embedding_size, batch_norm=True, last_activation=True, activation=activation, last_batchnorm=True),
                mar(embedding_size[-1], hidden_channel, ks=9, bias=True, activation=activation, bins=None, v=variable, channel_last=False, bn=True, residue=True),
                mar(hidden_channel, features, ks=winlen, stride=stride, bias=True, activation=activation, bins=None, v=variable, channel_last=False, bn=True),
                AddPositionalEmbedding(features,2001,rnn_type=rnn_type),
                *stackrnn(features, num_layers, rnn_type, transformer_activation, ln=False),
                LinearCRFEncoder(
                    features, n_base, state_len, activation='tanh', scale=scale,
                    blank_score=blank_score, expand_blanks=expand_blanks
                )
        ...
        raise ValueError("markonv should be 0 or 38.")
