...
def main():
    ...
    print('\t'.join(['Name', 'Length', 'Identity', 'Relative length', 'Strand']))
    for read_num, read_name in enumerate(read_names):
        ...
        for read_start, read_end, ref_start, ref_end, identity, strand in alignments:
        ...
        print('{}\t{}\t{:.5f}\t{}\t{}'.format(read_name, read_length, whole_read_identity, relative_length, "".join(sorted(list(possible_strands)))))
...
def load_alignments(paf_filename, read_lengths):
    ...
    with open(paf_filename, 'rt') as paf:
        for line in paf:
            ...
            read_alignments[read_name].append((read_start, read_end, ref_start, ref_end, identity, strand))
