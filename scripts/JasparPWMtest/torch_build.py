...
class LSTM(nn.Module):
    def __init__(self, in_channels, outputdim):
        super().__init__()
        self.LSTM = torch.nn.LSTM(input_size=in_channels, hidden_size=16, batch_first =True,bidirectional=True,bias=False)
        self.dropout = nn.Dropout(p=0.2)
        self.outputdim = outputdim
        self.linear = torch.nn.Linear(32000, outputdim)
...
def trainMarkonv(modelsave_output_prefix,data_set, number_of_kernel, kernel_size,
    ...
    #     return 0,0
        ...
        if outputName=="DANQ" or outputName=="LSTM":
