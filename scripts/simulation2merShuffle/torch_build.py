...
class DANQ(nn.Module):
    def __init__(self, in_channels,kernel_size, number_of_kernel, outputdim):
        super().__init__()
        self.Conv1d = torch.nn.Conv1d(in_channels, number_of_kernel, kernel_size, bias=False)
        self.LSTM = torch.nn.LSTM(input_size=number_of_kernel, hidden_size=number_of_kernel, batch_first =True,bidirectional=True,bias=False)
        self.maxpool1d = torch.nn.MaxPool1d(kernel_size=15, stride=15)
        self.dropout = nn.Dropout(p=0.2)
        self.outputdim = outputdim
        self.linear1 = torch.nn.Linear(16896, 512)
    ...
    def forward(self,x):
        x = x.permute((0,2,1)).contiguous().float()
        conv = self.Conv1d(x)
        pool = self.maxpool1d(conv)
        pool = pool.permute((0,2,1)).contiguous().float()
        lstm, (h_n, c_n) = self.LSTM(pool)
        flatten = torch.flatten(lstm, start_dim=1)
        output = self.dropout(flatten)
        if self.outputdim:
...
class DANQS(nn.Module):
    def __init__(self, in_channels,kernel_size, number_of_kernel, outputdim):
        super().__init__()
        self.Conv1d = torch.nn.Conv1d(in_channels, number_of_kernel, kernel_size, bias=False)
        self.LSTM = torch.nn.LSTM(input_size=number_of_kernel, hidden_size=number_of_kernel, batch_first =True,bidirectional=True,bias=False)
        self.maxpool1d = torch.nn.MaxPool1d(kernel_size=5, stride=5)
        self.dropout = nn.Dropout(p=0.2)
        self.outputdim = outputdim
        self.relu = torch.nn.ReLU()
        self.linear = torch.nn.Linear(number_of_kernel*2, outputdim)
        torch.nn.init.xavier_uniform_(self.Conv1d.weight)
    ...
    def forward(self,x):
        ...
        pool = pool.permute((0,2,1)).contiguous().float()
        lstm, (h_n, c_n) = self.LSTM(pool)
        flatten = torch.flatten(lstm[:,-1,:], start_dim=1)
        output = self.dropout(flatten)
        if self.outputdim:
            output = self.linear(output)
            output = torch.sigmoid(output)
...
class LSTM(nn.Module):
    def __init__(self, in_channels, outputdim):
        super().__init__()
    ...
    def forward(self,x):
        ...
        mar, (h_n, c_n) = self.LSTM(x)
        flatten = torch.flatten(mar, start_dim=1)
        output = self.dropout(flatten)
        if self.outputdim:
            output = self.linear1(output)
            output = self.relu(output)
...
class LSTM_last(nn.Module):
    def __init__(self, in_channels, outputdim):
        super().__init__()
        self.LSTM = torch.nn.LSTM(input_size=in_channels, hidden_size=512, batch_first=True,bidirectional=True,bias=False)
        self.dropout = nn.Dropout(p=0.2)
        self.outputdim = outputdim
        self.linear = torch.nn.Linear(1024, outputdim)
        self.relu = torch.nn.ReLU()

    def forward(self,x):
        x = x.contiguous().float()
        mar, (h_n, c_n) = self.LSTM(x)
        # flatten = torch.flatten(mar[:,-1,:], start_dim=1)
        output = self.dropout(mar[:,-1,:])
        if self.outputdim:
            output = self.linear(output)
            output = torch.sigmoid(output)
        return output.squeeze()
...
def trainMarkonv(modelsave_output_prefix,data_set, number_of_kernel, kernel_size,
    ...
    elif outputName == "LSTM":
        net = LSTM(training_set[0].shape[2],1)
    elif outputName == "LSTM_last":
        net = LSTM_last(training_set[0].shape[2],1)
        # batch_size = int(batch_size/2)
    elif outputName == "DANQ":
        net = DANQ(training_set[0].shape[2],kernel_size, number_of_kernel, 1)
    elif outputName == "DANQS":
        net = DANQS(training_set[0].shape[2], kernel_size, number_of_kernel, 1)
    else:
    ...
    #     return 0,0
        ...
        if outputName=="DANQ" or outputName=="LSTM" or outputName=="DANQS":
